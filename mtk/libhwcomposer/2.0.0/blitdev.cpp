...
BlitDevice::BlitDevice()
    ...
    , m_last_input_ion_fd(-1)
    , m_last_output_ion_fd(-1)
{
...
BlitDevice::~BlitDevice()
{
    ...
    if (m_last_input_ion_fd != -1)
        IONDevice::getInstance().ionCloseAndSet(&m_last_input_ion_fd);

    if (m_last_output_ion_fd != -1)
        IONDevice::getInstance().ionCloseAndSet(&m_last_output_ion_fd);
    ...
    int dpy, OverlayPrepareParam* param)
{
    ...
    IONDevice::getInstance().ionImport(param->ion_fd, &m_last_input_ion_fd);
    {
        AutoMutex l(m_vector_lock);
    ...
    int dpy, OverlayPortParam* const* params, int /*num*/, sp<ColorTransform> /*color_transform*/)
{
    ...
    int src_ion_fd = m_last_input_ion_fd;
...
void BlitDevice::prepareOverlayOutput(int dpy, OverlayPrepareParam* param)
{
    ...
    IONDevice::getInstance().ionImport(param->ion_fd, &m_last_output_ion_fd, "BlitDevice::prepareOverlayOutput()");
...
void BlitDevice::enableOverlayOutput(int dpy, OverlayPortParam* param)
{
    ...
    DpSecure is_dp_secure = DP_SECURE_NONE;
    int dst_ion_fd = m_last_output_ion_fd;
