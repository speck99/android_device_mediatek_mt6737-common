...
DispDevice::DispDevice()
    : m_last_valid_color_transform(nullptr)
    , m_prev_enable_ccorr(false)
{
    char filename[256];
    ...
    int dpy, OverlayPortParam* const* params, int num, sp<ColorTransform> color_transform)
{
    ...
    if (Platform::getInstance().m_config.support_color_transform)
    {
        ...
        if (enable_ccorr)
        {
            ...
            {
                if (m_prev_enable_ccorr == false && m_last_valid_color_transform != nullptr)
                {
                    m_frame_cfg[dpy].ccorr_config.is_dirty = true;
                    m_frame_cfg[dpy].ccorr_config.mode = m_last_valid_color_transform->hint;
                    const int32_t dim = 4;
                    for (int32_t j = 0; j < dim * dim ; ++j)
                        m_frame_cfg[dpy].ccorr_config.color_matrix[j] = static_cast<int32_t>(m_last_valid_color_transform->matrix[j / 4][j % 4] * 1024.f + 0.5f);
                }
                else
                {
        ...
        m_prev_enable_ccorr = enable_ccorr;
        if (color_transform != nullptr)
            m_last_valid_color_transform = color_transform;
