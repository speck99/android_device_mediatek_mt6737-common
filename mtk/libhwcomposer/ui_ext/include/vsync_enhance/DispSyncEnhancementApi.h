...
typedef std::function<status_t(const char*, nsecs_t, const sp<DispSync::Callback>&)> AddEventListener;
typedef std::function<status_t(void)> HasAnyEventListeners;
typedef std::function<void(void)> EnableHardwareVsync;
typedef std::function<status_t(const sp<DispSync::Callback>& callback)> RemoveEventListener;
...
public:
    ...
    virtual bool addEventListener(status_t* /*res*/, Mutex* /*mutex*/, const char* /*name*/,
                                  nsecs_t /*phase*/, const sp<DispSync::Callback>& /*callback*/) { return false; }

    // used to remove event listener
    virtual bool removeEventListener(status_t* /*res*/, Mutex* /*mutex*/,
                                      const sp<DispSync::Callback>& /*callback*/) { return false; }
