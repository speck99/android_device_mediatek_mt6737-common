...
#define ION_HEAP_MULTIMEDIA_SEC_MASK (1 << 13)
...
static int _ge_query_ion_debug(buffer_handle_t hnd, int UNUSED(ge_fd), void *out)
{
	int err;
...
#undef GE_INOUT
#define GE_INOUT out
int ge_query(buffer_handle_t hnd, int ge_fd, GRALLOC_EXTRA_ATTRIBUTE_QUERY attr, void *out)
{
	ge_info gei;
	switch (attr)
	{
	...
	case 100:
		gei = GEI_BY_STRUCT(ge_sf_info_t);
		_ge_get(ge_fd, &gei, out);
		return 0;
		break;

	case 108:
		gei = GEI_BY_STRUCT(ge_hwc_info_t);
		_ge_get(ge_fd, &gei, out);
		return 0;
		break;

	case 106:
		gei = GEI_BY_STRUCT(ge_hdr_info_t);
		_ge_get(ge_fd, &gei, out);
		return 0;
		break;

	case 107:
		gei = GEI_BY_STRUCT(ge_video_info_t);
		_ge_get(ge_fd, &gei, out);
		return 0;
		break;

	case 110:
		gei = GEI_BY_STRUCT(ge_timestamp_info_t);
		_ge_get(ge_fd, &gei, out);
		return 0;
		break;

	case 102:
		gei = GEI_BY_MEMBER(ge_sec_hwc_t, sec_hnd);
		_ge_get(ge_fd, &gei, out);
		return 0;
		break;

	case 105:
		gei = GEI_BY_MEMBER(ge_misc_info_t, orientation);
		_ge_get(ge_fd, &gei, out);
		return 0;
		break;

	case 109:
		gei = GEI_BY_MEMBER(ge_timestamp_info_t, timestamp);
		_ge_get(ge_fd, &gei, out);
		return 0;
		break;
...
int ge_perform(buffer_handle_t hnd, GEFD ge_fd, GRALLOC_EXTRA_ATTRIBUTE_PERFORM attr, void *in)
{
	...
	switch (attr)
	{
	case GRALLOC_EXTRA_SET_SF_INFO:
		gei = GEI_BY_STRUCT(ge_sf_info_t);
		_ge_set(ge_fd, &gei, in);
		return 0;
		break;

	case GRALLOC_EXTRA_SET_HWC_INFO:
		gei = GEI_BY_STRUCT(ge_hwc_info_t);
		_ge_set(ge_fd, &gei, in);
		return 0;
		break;

	case GRALLOC_EXTRA_SET_HDR_INFO:
		gei = GEI_BY_STRUCT(ge_hdr_info_t);
		_ge_set(ge_fd, &gei, in);
		return 0;
		break;

	case GRALLOC_EXTRA_SET_VIDEO_INFO:
		gei = GEI_BY_STRUCT(ge_video_info_t);
		_ge_set(ge_fd, &gei, in);
		return 0;
		break;

	case GRALLOC_EXTRA_SET_ORIENTATION:
		gei = GEI_BY_MEMBER(ge_misc_info_t, orientation);
		_ge_set(ge_fd, &gei, in);
		return 0;
		break;

	case GRALLOC_EXTRA_SET_TIMESTAMP:
		gei = GEI_BY_MEMBER(ge_timestamp_info_t, timestamp);
		_ge_set(ge_fd, &gei, in);
		return 0;
		break;

	case GRALLOC_EXTRA_SET_TIMESTAMP_INFO:
		gei = GEI_BY_STRUCT(ge_timestamp_info_t);
		_ge_set(ge_fd, &gei, in);
		return 0;
		break;
